...
> **Note**: a similar report is generated by the `aot_arm_compiler.py` script for lowering PyTorch models to Ethos-U,
> which invokes Vela internally.

The `Total SRAM used` here shows the required memory to store the `tensor arena` for the ML framework.
This is the amount of memory required to store the input, output, and intermediate buffers.
In the preceding example, the tensor arena requires 146.31 KiB of available SRAM.

> **Note:** Vela can only estimate the SRAM required for graph execution.
> It has no way of estimating the memory used by internal structures from the
> TensorFlow Lite Micro or ExecuTorch frameworks.
...
To use the other supported memory modes refer to [vela.ini](https://gitlab.arm.com/artificial-intelligence/ethos-u/ethos-u-vela/-/blob/5.0.0/ethosu/config_files/Arm/vela.ini).
...
Although the Vela settings for this configuration suggests that only AXI0 bus is used, when compiling the model
...
and compute buffer region in the ExecuTorch framework.
...
> **Note:** The SRAM usage does not include memory used by the ML framework and must be topped up as explained
