/*
 * SPDX-FileCopyrightText: Copyright 2021-2022, 2024-2025 Arm Limited and/or its affiliates <open-source-office@arm.com>
...
__attribute__((section(".bss.tensor_arena"), aligned(32))) uint8_t TFLuTensorArena[TENSOR_ARENA_SIZE];
                           ...
                           const uint32_t len_ns)      /* Length (in bytes) of non-secure region */
{
    ...
    if (MPC_SIE_ERR_NONE != ret) {
        LOG_ERR("Error configuring non-secure region for %s (%d)", mem_name, ret);
        return 1;
    }
    ...
    if (MPC_SIE_ERR_NONE != ret) {
        LOG_ERR("Error configuring secure region for %s", mem_name);
        return 1;
    }
    ...
    if (MPC_SIE_ERR_NONE != ret) {
        LOG_ERR("Error locking down MPC for %s", mem_name);
        return 1;
    }
                          ...
                          const uint32_t len_ns)      /* Length (in bytes) of non-secure region */
{
    ...
    ret = mpc_sie_config_region(&mpc_dev, baseaddr_s, baseaddr_s + len_s - 1, MPC_SIE_SEC_ATTR_SECURE);
    if (MPC_SIE_ERR_NONE != ret) {
        LOG_ERR("Error configuring secure region for %s (%d)", mem_name, ret);
        return 1;
    }
    ...
    ret = mpc_sie_lock_down(&mpc_dev);
    if (MPC_SIE_ERR_NONE != ret) {
        LOG_ERR("Error locking down MPC for %s (%d)", mem_name, ret);
        return 1;
    }
...
void setup_xtgu_ns(uint32_t xtgu_base, uint32_t xtcm_start, uint32_t xtcm_size) {
    ...
    if (xtcm_start % BLKSZ != 0)
        LOG_WARN("XTCM: start address %08x not on block size boundary", xtcm_start);

    if ((xtcm_start + xtcm_size) % BLKSZ != 0)
        LOG_WARN("XTCM: limit address %08x not on block size boundary", xtcm_start + xtcm_size);

    LOG_INFO("setting up xTGU LUT for mem@%08x(%08x)", xtcm_start, xtcm_size);
    ...
    while (xtcm_address < xtcm_end) {
        ...
        if (block_idx >= MAX_BLK_NBR) {
            LOG_ERR("lut bit mask too small, aborting!");
            exit(1);
        }
...
int setup_secure_attributes(void) {
    ...
    if (res) {
        LOG_ERR("BRAM MPC Failed");
    } else {
        LOG_INFO("BRAM MPC OK");
    }
    ...
    if (res) {
        LOG_ERR("SRAM MPC Failed");
    } else {
        LOG_INFO("SRAM MPC OK");
    }
    ...
    *NSCCFG          = 0x1;
    LOG_INFO("NSCCFG set NSC for CODE SRAM 0x10000000-0x1fffffff");
...
void boot_non_secure() {
    ...
    LOG_INFO("Setting NS MSP : 0x%x", *(uint32_t *)TZ_NS_START_VECTOR);
    /* Setup non-secure stack */
    __TZ_set_MSP_NS(*(uint32_t *)TZ_NS_START_VECTOR);

    /* Setup non-secure reset vector */
    LOG_INFO("Setting NS VTOR : 0x%x", TZ_NS_START_VECTOR);
    ...
    LOG_INFO("Setting NS_ResetHandler to: %p", NS_ResetHandler);
...
int main() {
    int ret = -1;
    LOG_INFO("Secure main starting up.");
    ...
    LOG_INFO("We're back in secure world!");
...
int run_inference(void) {
    ...
    LOG("Status of executing the job: %s", failed ? "failed\n" : "success\n");
