...
// Default to 32 byte alignment for tensor arena due to cache line size
__attribute__((section(".bss.tensor_arena"), aligned(32)))
...
void *ethosu_mutex_create(void) {
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("mutex create failed, error - %u", status);
    }
    return (void *)mutex;
}
...
int ethosu_mutex_lock(void *mutex) {
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("mutex get failed, error - %u", status);
        return -1;
    }
    return 0;
}
...
int ethosu_mutex_unlock(void *mutex) {
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("mutex put failed, error - %u", status);
        return -1;
    }
    return 0;
}
...
void *ethosu_semaphore_create(void) {
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("Semaphore create failed, error - %u", status);
    }
...
int ethosu_semaphore_take(void *sem, uint64_t timeout) {
    ...
    if (status != TX_SUCCESS) {
        if (status != TX_NO_INSTANCE) {
            LOG_ERR("Semaphore get/take, error - %u", status);
        }
        return -1;
    }
...
int ethosu_semaphore_give(void *sem) {
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("Semaphore put/give, error - %u", status);
        return -1;
    }
...
void inferenceProcessThread(ULONG pvParameters) {
    ...
    for (;;) {
        ...
        if (tx_status != TX_SUCCESS) {
            LOG_ERR("process failed to receive from Queue, error - %u", tx_status);
            exit(1);
        }
        ...
        if (tx_status != TX_SUCCESS) {
            LOG_ERR("process inferenceProcessThread failed to send to Queue, error - %u", tx_status);
            exit(1);
        }
    }
    ...
    if (tx_status != TX_SUCCESS) {
        LOG_ERR("process inferenceProcessThread failed to terminate thread, error - %u", tx_status);
        exit(1);
    }
}
...
void inferenceSenderThread(ULONG pvParameters) {
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("Sender thread failed to allocate bytes for Queue, error - %u", status);
        exit(1);
    }
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("Sender thread failed to create Queue, error - %u", status);
        exit(1);
    }
    ...
    for (int n = 0; n < NUM_JOBS_PER_THREAD; n++) {
        ...
        if (status != TX_SUCCESS) {
            LOG_ERR("Sender thread failed to send to Queue, error - %u", status);
            exit(1);
        }
    }
    ...
    do {
        ...
        if (status != TX_SUCCESS) {
            LOG_ERR("Sender thread failed to receive from Queue, error - %u", status);
            exit(1);
        }
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("Sender thread failed to delete Queue, error - %u", status);
        exit(1);
    }
...
void tx_application_define(void *first_unused_memory) {
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("Main failed to allocate pool of bytes, error - %u", status);
        exit(1);
    }
    ...
    if (status != TX_SUCCESS) {
        LOG_ERR("Main failed to allocate bytes for process queue, error - %u", status);
        exit(1);
    }
    ...
    status = tx_queue_create(&inferenceProcessQueue,
                             ...
                             PROCESS_QUEUE_SIZE);
    if (status != TX_SUCCESS) {
        LOG_ERR("Main failed to create Queue, error - %u", status);
        exit(1);
    }
    ...
    for (int n = 0; n < NUM_JOB_THREADS; n++) {
        ...
        if (status != TX_SUCCESS) {
            LOG_ERR("Main failed to allocate bytes for sender tread, error - %u", status);
            exit(1);
        }
        ...
        if (status != TX_SUCCESS) {
            LOG_ERR("Main failed to allocate bytes for sender tread stack, error - %u", status);
            exit(1);
        }
        ...
        status = tx_thread_create(reinterpret_cast<TX_THREAD *>(senderThreadPtr[n]),
                                  ...
                                  TX_AUTO_START);
        if (status != TX_SUCCESS) {
            LOG_ERR("Main failed to create Thread, error - %u", status);
            exit(1);
        }
    }
    ...
    for (int n = 0; n < NUM_INFERENCE_THREADS; n++) {
        ...
        if (status != TX_SUCCESS) {
            LOG_ERR("Main failed to allocate bytes for process tread, error - %u", status);
            exit(1);
        }
        ...
        if (status != TX_SUCCESS) {
            LOG_ERR("Main failed to allocate bytes for process stack, error - %u", status);
            exit(1);
        }
    ...
    LOG_INFO("ThreadX application initialisation - Done");
    return;
}
