...
void *ethosu_semaphore_create(void) {
    ...
    if (sem == NULL) {
        LOG_ERR("Error: Failed to create semaphore.");
    }
    return (void *)sem;
}
...
void inferenceProcessTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (xQueueReceive(params.queueHandle, &xJob, portMAX_DELAY) != pdPASS) {
            LOG_ERR("inferenceProcessTask failed in receive from Q.");
            exit(1);
        }
        ...
        if (xQueueSend(xJob->responseQueue, &xJob, portMAX_DELAY) != pdPASS) {
            LOG_ERR("inferenceProcessTask failed in send to Q.");
            exit(1);
        }
    }
    vTaskDelete(nullptr);
...
void inferenceSenderTask(void *pvParameters) {
    ...
    for (int n = 0; n < NUM_JOBS_PER_TASK; n++) {
        ...
        // Send job
        LOG_INFO("inferenceSenderTask: Sending inference job: job=%p, name=%s", job, job->name.c_str());
        if (xQueueSend(_inferenceProcessQueue, &job, portMAX_DELAY) != pdPASS) {
            LOG_ERR("inferenceSenderTask failed in send to Q.");
            exit(1);
        }
    }
    ...
    do {
        ...
        if (xQueueReceive(senderQueue, &pSendJob, portMAX_DELAY) != pdPASS) {
            LOG_ERR("inferenceSenderTask failed in receive from Q.");
            exit(1);
        }
        LOG_INFO("inferenceSenderTask: received response for job: %s, status = %d",
                 pSendJob->name.c_str(),
                 pSendJob->status);
    ...
    LOG("FreeRTOS application returning %d.\n", ret);
    exit(ret);
}
...
int main() {
    ...
    for (int n = 0; n < NUM_JOB_TASKS; n++) {
        ...
        if (ret != pdPASS) {
            LOG_ERR("Failed to create 'inferenceSenderTask%i'", n);
            exit(1);
        }
    }
    ...
    for (int n = 0; n < NUM_INFERENCE_TASKS; n++) {
        ...
        if (ret != pdPASS) {
            LOG_ERR("Failed to create 'inferenceProcessTask%i'", n);
            exit(1);
        }
    }
    ...
    LOG_ERR("FreeRTOS application failed to initialise.");
    exit(1);
