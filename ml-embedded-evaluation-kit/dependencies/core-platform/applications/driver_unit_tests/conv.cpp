...
int main() {
    ...
    LOG("Running %u iterations using sync API...\n", repeat);
    ...
    if (res == 0) {
        ...
        if (outputPointer != expectedPointer) {
            LOG("Output mismatch for sync test\n");
            res = 1;
        }
    }
    errors = res;
    ...
    LOG("Running %u iterations using async API...\n", repeat);
    ...
    for (size_t r = repeat; r > 0; r--) {
        ...
        if (res != 0) {
            LOG("Async inference test failed\n");
            errors++;
            break;
        }
    }
    ...
    if (res == 0) {
        ...
        // Compare output with expected data
        if (outputPointer != expectedPointer) {
            LOG("Output mismatch for async test\n");
            errors++;
        }
    }
    LOG("\n");
