...
std::shared_ptr<RProc> rproc;
std::shared_ptr<MessageHandler> messageHandler;
std::array<std::shared_ptr<InferenceRunner>, NUM_PARALLEL_TASKS> inferenceRunner;

void setupTask(void *) {
    LOG_DEBUG("Running setup task");

    rproc          = std::make_shared<RProc>(mailbox, resourceTable.table, sizeof(resourceTable));
    messageHandler = std::make_shared<MessageHandler>(*rproc, "ethos-u-0.0");

    LOG("TFLM arena. pa=%" PRIx32 ", da=%" PRIx32 ", len=%" PRIx32 "\n",
    ...
    // This task is not needed after the setup is done
    vTaskDelete(NULL);
}
...
int main() {
    LOG("Ethos-U Message Handler OpenAMP\n");

    TaskHandle_t taskHandle;
    BaseType_t ret = xTaskCreate(setupTask, "setupTask", 1024, nullptr, 4, &taskHandle);
    if (ret != pdPASS) {
